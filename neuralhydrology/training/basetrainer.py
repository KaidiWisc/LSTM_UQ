...
import pyro
from pyro.infer import JitTrace_ELBO,Predictive,Trace_ELBO
from pyro.optim import Adam
from pyro.infer import SVI
from pyro.infer.autoguide import AutoNormal
import pyro.poutine as poutine
from variational_dist import var_dist_define
        ...
        if isinstance(self.cfg.finetune_modules, list):
            for module_part in self.cfg.finetune_modules:
                if module_part in self.model.module_parts:
                    ...
                    if module_part=="lstms": # only additional opt when finetune lstms
                        if self.cfg.finetune_params=="prcp":
                            pla=0
                            for param in module.parameters():
                                print(param.size())
                                if (pla==0) | (pla==2) | (pla==4) | (pla==6):
...
#!!!
        ...
        if self.cfg.is_finetuning: #self.model.parameters():
...
#
        ...
        if self.cfg.bvi==1:
            self.vdist=var_dist_define(self.model,cfg=self.cfg, init_scale=self.cfg.bvisigma)
            self.svi = SVI(model=self.model, guide = self.vdist, #AutoNormal(self.model),
                           optim = Adam({"lr": self.cfg.learning_rate[0]}), loss=Trace_ELBO(), num_chains=4)
    ...
    def train_and_validate(self):
        ...
        for epoch in range(self._epoch + 1, self._epoch + self.cfg.epochs + 1):
            ...
            if self.cfg.bvi==0:
                ...
                self._train_epoch_BVI(epoch=epoch)
                print(f"Epoch {epoch}, Loss: {self.bviloss}")
                self.bviloss=0
            ...
            if (epoch % self.cfg.save_weights_every == 0) :
                ...
                # Save the parameters of the guide
                
            if (epoch % self.cfg.save_weights_every == 0) & (self.cfg.bvi==1):
                # torch.save(self.svi.guide.state_dict(), 'guide_parameters_%d.pth'%(epoch))  
                # torch.save(self.svi.model.state_dict(), 'model_parameters_%d.pth'%(epoch))  
                pyro.get_param_store().save(self.cfg.run_dir/f"model_guide_params_{epoch}.pyro")
        ...
        if self.cfg.bvi==1:
            self.vdist.requires_grad_(False)     
    ...
    def _save_weights_and_optimizer(self, epoch: int):
        ...
        if self.cfg.bvi==1: #!!!
            ...
            if self.cfg.model=="mtslstm":
                modelist['lstms']=self.model.lstms                
                modelist['transfer_fcs']=self.model.transfer_fcs
                modelist['heads']=self.model.heads
                
            else:
                modelist['lstms']=self.model.lstm
                modelist['heads']=self.model.head
                
            torch.save(modelist, str(weight_path))
    ...
    def _train_epoch(self, epoch: int):
        ...
        for i, data in enumerate(pbar):
            ...
            # get predictions self.model.lstms["1D"]._flat_weights
            ...
            if torch.isnan(loss):
                ...
                # delete old gradients, first iter, all 0.
                ...
                # update weights self.model.lstms["1D"]._flat_weights
                
                self.optimizer.step() #!!! adam step
    ...
    def _train_epoch_BVI(self, epoch: int):
        ...
        for i, data in enumerate(pbar):
            ...
            if torch.isnan(loss):
                ...
                # svi = SVI(model=self.model, guide = AutoNormal(self.model), optim=self.optimizer, loss=JitTrace_ELBO(), num_chains=4)
                data_={}
                data_ = {k: v for k, v in data.items() if "date" not in k}
                svi_loss = self.svi.step(data_) # do every update, check whether update model
                self.bviloss+=svi_loss
