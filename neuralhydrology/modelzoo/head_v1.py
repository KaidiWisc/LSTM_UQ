...
def get_head(cfg: Config, n_in: int, n_out: int, freq: str) -> nn.Module:
    ...
    if cfg.head.lower() == "regression":
        ...
        head = UMAL(n_in=n_in, n_out=n_out, bvi=cfg.bvi, freq=freq)
    elif cfg.head.lower() == "cmal":
        head = CMAL(n_in=n_in, n_out=n_out, bvi=cfg.bvi, freq=freq)
    elif cfg.head.lower() == "":
...
class Regression(nn.Module):
    ...
    def __init__(self, n_in: int, n_out: int, activation: str = "linear", bvi: bool = 0, freq: str="1H"):
        ...
        if bvi ==0:
            ...
            layer0.bias = PyroSample(dist.Normal(0., 0.1).expand([ n_out]).to_event(1)) #,name=f"head_bias_{freq}")
            layer0.weight = PyroSample(dist.Normal(0., 0.1).expand([ n_out, n_in]).to_event(2)) #,name=f"head_weight_{freq}")
...
class GMM(nn.Module):
    ...
    def __init__(self, n_in: int, n_out: int, n_hidden: int = 100, bvi: bool = 0, freq: str="1H"):
        ...
        if bvi==0:
            ...
            self.fcs[f"fc1_{freq}"].bias = PyroSample(dist.Normal(0., 0.1).expand([ n_hidden]).to_event(1)) #,name=f"head_fc1_bias_{freq}")
            self.fcs[f"fc1_{freq}"].weight = PyroSample(dist.Normal(0., 0.1).expand([n_hidden, n_in]).to_event(2)) #,name=f"head_fc1_weight_{freq}")    
            
            self.fcs[f"fc2_{freq}"] = PyroModule[nn.Linear](n_hidden, n_out)
            self.fcs[f"fc2_{freq}"].bias = PyroSample(dist.Normal(0., 0.1).expand([n_out]).to_event(1)) #,name=f"head_fc2_bias_{freq}")
            self.fcs[f"fc2_{freq}"].weight = PyroSample(dist.Normal(0., 0.1).expand([n_out, n_hidden]).to_event(2)) #,name=f"head_fc2_weight_{freq}")
