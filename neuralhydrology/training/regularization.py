...
class TiedFrequencyMSERegularization(BaseRegularization):
    ...
    def forward(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor],
        ...
        from neuralhydrology.utils.samplingutils import _sample_gaussian_mixtures,_handle_negative_values
        loss = 0
        
        if self.cfg.head=="gmm":
            for freq in self._frequencies:
                m_sub=prediction[f'mu_{freq}']
                p_sub=prediction[f'pi_{freq}']   
                prediction[f'y_hat_{freq}']=torch.sum(m_sub*p_sub,2,keepdim=True)
        ...
        if self.cfg.head=="umal":  
            for freq in self._frequencies:
                m_sub=prediction[f'mu_{freq}'] 
                prediction[f'y_hat_{freq}']=torch.mean(m_sub,2,keepdim=True)
