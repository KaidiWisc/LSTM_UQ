...
class MTSLSTM(BaseModel):
    ...
    def __init__(self, cfg: Config):
        ...
        self.activ=cfg.output_activation
    ...
    def _init_modules_BVI_ini(self, input_sizes: Dict[str, int]):
        ...
        for idx, freq in enumerate(self._frequencies):
            ...
            if self._is_shared_mtslstm and idx > 0:
                ...
                self.heads[freq] = copy.deepcopy(self.heads[self._frequencies[idx - 1]])   # same head for all frequencies.
    ...
    def _init_modules_BVI(self, input_sizes: Dict[str, int]):
        ...
        for idx, freq in enumerate(self._frequencies):
            ...
            # I will manually set their values are identical
            if self._is_shared_mtslstm and idx > 0:
                import copy
                # lstm are corelated, both pyro and changes togother, but lstms[1h] never applies
                self.lstms[freq] =self.lstms[self._frequencies[idx - 1]]    # same LSTM for all frequencies.
                
                # heads ae indenpend, one->nn.linear, one->pyro
                self.heads[freq] = copy.deepcopy(self.heads[self._frequencies[idx - 1]])   # same head for all frequencies.
            else:
                self.lstms[freq] = PyroModule[nn.LSTM](input_size=freq_input_size, hidden_size=self._hidden_size[freq]).to(self.device)  #!!!
                # Define Prior Distributions for LSTM Parameters               
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        outputs = {}
        for idx, freq in enumerate(self._frequencies):
            if idx < len(self._frequencies) - 1: # D
                ...
                if self._is_shared_mtslstm==1: #
                    ...
                    lstm_output, _ = lstm_copy(x_d[freq], (h_0_transfer, c_0_transfer))
                    
                else:
                    lstm_output, _ =  self.lstms[freq](x_d[freq], (h_0_transfer, c_0_transfer))

            if (self._is_shared_mtslstm==0): # do itself
                head_out = self.heads[freq](self.dropout(lstm_output.transpose(0, 1)))
                
            if (self._is_shared_mtslstm==1):  # do one
                if idx==0: # do it H
                    head_out = self.heads[freq](self.dropout(lstm_output.transpose(0, 1)))
                ...
                if idx==1: # copy it D
                    ...
                    if self.headname=="regression": 
                        # replace head to be NOTpyro
                        # note that initial head is pyro, so that keys are using bvi==1

                        self.heads[freq].net[f"l0_1D"]=nn.Linear(self._hidden_size[freq], self.output_size).to(self.device)   
                        # I do not need identical value, but fromidetical distri,
                        self.heads[freq].net[f"l0_1D"].bias.data.copy_(self.heads[self._frequencies[0]].net[f"l0_1D"].bias)    
                        self.heads[freq].net[f"l0_1D"].weight.data.copy_(self.heads[self._frequencies[0]].net[f"l0_1D"].weight) 
                        
                        head_out = self.heads[freq](self.dropout(lstm_output.transpose(0, 1)))
                
                    if (self.headname=="gmm") or (self.headname=="cmal") or (self.headname=="umal") :
                        
                        self.heads[freq].fcs[f"fc1_1D"]=nn.Linear(self._hidden_size[freq], 100).to(self.device) 
                        self.heads[freq].fcs[f"fc1_1D"].bias.data.copy_(self.heads[self._frequencies[0]].fcs[f"fc1_1D"].bias)
                        self.heads[freq].fcs[f"fc1_1D"].weight.data.copy_(self.heads[self._frequencies[0]].fcs[f"fc1_1D"].weight)
                        
                        self.heads[freq].fcs[f"fc2_1D"] = nn.Linear(100,self.output_size).to(self.device)
                        self.heads[freq].fcs[f"fc2_1D"].bias.data.copy_(self.heads[self._frequencies[0]].fcs[f"fc2_1D"].bias)
                        self.heads[freq].fcs[f"fc2_1D"].weight.data.copy_(self.heads[self._frequencies[0]].fcs[f"fc2_1D"].weight)
