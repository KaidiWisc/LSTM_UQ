...
def sample_gmm(model: 'BaseModel', data: Dict[str, torch.Tensor], n_samples: int,
 ...
 #-----------warm-up-------------------------------------         
        ...
        del m0,m_target0
        m_sub=torch.zeros((m_sub0.shape[0]*nn_samples, m_sub0.shape[1],(setup.number_of_targets),m_sub0.shape[2])) 
        del m_sub0
        ...
        for nth_target in range(setup.number_of_targets):
            ...
            for ii in range(int(nn_samples)):
                ...
                if any(mask_nan):  # skip if the complete mini-batch is invalid
                    m_sub1 = torch.repeat_interleave(m_target[mask_nan, :, :], n_samples, dim=0)
                    setlen=m_sub1.shape[0]
                    m_sub[pp:pp+setlen,:,nth_target,:] = torch.repeat_interleave(m_target[mask_nan, :, :], n_samples, dim=0)
                    s_sub[pp:pp+setlen,:,nth_target,:] = torch.repeat_interleave(s_target[mask_nan, :, :], n_samples, dim=0)
                    p_sub[pp:pp+setlen,:,nth_target,:] = torch.repeat_interleave(p_target[mask_nan, :, :], n_samples, dim=0)
                    pp=pp+setlen
...
def sample_cmal(model: 'BaseModel', data: Dict[str, torch.Tensor], n_samples: int,
 ...
 #-----------warm-up-------------------------------------         
            ...
            del m0,m_target_0
            m_target=torch.zeros((m_target0.shape[0]*nn_samples, m_target0.shape[1],m_target0.shape[2])) 
            
            setlen=m_target0.shape[0]
            del m_target0
            b_target=torch.zeros(m_target.shape)
            t_target=torch.zeros(m_target.shape)
            p_target=torch.zeros(m_target.shape)
            ...
            for ii in range(int(nn_samples)):
                ...
                m_target[ii*setlen:(ii+1)*setlen,:,:] = torch.repeat_interleave(m_target_, nn_samples, dim=0)
                b_target[ii*setlen:(ii+1)*setlen,:,:] = torch.repeat_interleave(b_target_, nn_samples, dim=0)
                t_target[ii*setlen:(ii+1)*setlen,:,:] = torch.repeat_interleave(t_target_, nn_samples, dim=0)
                p_target[ii*setlen:(ii+1)*setlen,:,:] = torch.repeat_interleave(p_target_, nn_samples, dim=0)
