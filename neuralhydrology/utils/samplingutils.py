...
def sample_mcd(model: 'BaseModel', data: Dict[str, torch.Tensor], n_samples: int,
    ...
    for freq_suffix in setup.freq_suffixes:
        ...
        for nth_target in range(setup.number_of_targets):
            ...
            if model.cfg.svitest==0: # just use the loaded weight for further eva
                target_values = _sample_values(ids)
 
#--------------------------------------------------------------------------------------------------
            if model.cfg.svitest==1: # model is already, but disgard it here and rebuild model ens !!!
                ...
                pyro.get_param_store().load(model.cfg.svimodel,map_location=model.cfg.device)
                ...
                for i in range(n_samples):
                    ...
                    for name, value in param_store.items():
                        if "loc" in name:  # Mean of a Gaussian parameter
                            param_name = name.replace("var_dist_define.locs.", "")
                            scale_name = name.replace(".loc", ".scale") 
                            
                            if scale_name in param_store:
                                mean = value
                                scale = param_store[scale_name]
                                sampled_value = dist.Normal(mean, scale).sample()
                            else:
                                sampled_value = value  # If no scale, treat as deterministic
                            
                            sampled_params[param_name] = sampled_value
                    
                    
                    if setup.cfg.model=="mtslstm":
                        
                        if setup.cfg.shared_mtslstm==0:
                            
                            for ff in setup.cfg.use_frequencies:
                                
                                if setup.cfg.lstmfix==0:
                                    model.lstms[f"{ff}"].weight_ih_l0=sampled_params[f'{ff}.weight_ih_l0']
                                    model.lstms[f"{ff}"].weight_hh_l0=sampled_params[f'{ff}.weight_hh_l0']
                                    model.lstms[f"{ff}"].bias_ih_l0=sampled_params[f'{ff}.bias_ih_l0']
                                    model.lstms[f"{ff}"].bias_hh_l0=sampled_params[f'{ff}.bias_hh_l0']  
                                
                                if setup.cfg.headfix==0:
                                    model.heads[f"{ff}"].net[f'l0_{ff}'].weight=sampled_params[f"l0_{ff}.weight"]
                                    model.heads[f"{ff}"].net[f'l0_{ff}'].bias=sampled_params[f"l0_{ff}.bias"]    
                                                    
                            if setup.cfg.transferfix==0:
                                model.transfer_fcs["c_1D"].weight=sampled_params['c_1D.weight']
                                model.transfer_fcs["c_1D"].bias=sampled_params['c_1D.bias']
                                model.transfer_fcs["h_1D"].weight=sampled_params['h_1D.weight']
                                model.transfer_fcs["h_1D"].bias=sampled_params['h_1D.bias']
                        else:
                            
                            for ff in setup.cfg.use_frequencies:
                                
                                if setup.cfg.lstmfix==0:
                                    model.lstms[f"{ff}"].weight_ih_l0=sampled_params[f'1H.weight_ih_l0']
                                    model.lstms[f"{ff}"].weight_hh_l0=sampled_params[f'1H.weight_hh_l0']
                                    model.lstms[f"{ff}"].bias_ih_l0=sampled_params[f'1H.bias_ih_l0']
                                    model.lstms[f"{ff}"].bias_hh_l0=sampled_params[f'1H.bias_hh_l0']  
                                
                                if setup.cfg.headfix==0:
                                    model.heads[f"{ff}"].net[f'l0_1D'].weight.data=sampled_params[f"l0_1D.weight"]
                                    model.heads[f"{ff}"].net[f'l0_1D'].bias.data=sampled_params[f"l0_1D.bias"]    
                    
                        
                    else:
                        model.lstm.weight_ih_l0=sampled_params['weight_ih_l0']
                        model.lstm.weight_hh_l0=sampled_params['weight_hh_l0']
                        model.lstm.bias_ih_l0=sampled_params['bias_ih_l0']
                        model.lstm.bias_hh_l0=sampled_params['bias_hh_l0']                    
                        model.head.net['l0_1D'].weight=sampled_params["l0_1D.weight"]
                        model.head.net['l0_1D'].bias=sampled_params["l0_1D.bias"]
...
#--------------------------------------------------------------------------------------------------
            ...
            values = _handle_negative_values(setup.cfg, target_values, _sample_values, scaler, nth_target)
...
def sample_gmm(model: 'BaseModel', data: Dict[str, torch.Tensor], n_samples: int,
    ...
    if (setup.mc_dropout) or (setup.cfg.svitest):

        nn_samples =int(np.sqrt(n_samples))        
        pred = model(data)    
        ...
        for freq_suffix in setup.freq_suffixes:
            ...
            pred = model(data)
            m0 = pred[f'mu{freq_suffix}']
            del pred
            
            mset=torch.zeros(( m0.shape[0], m0.shape[1],m0.shape[2],nn_samples))
            del m0
            sset=torch.zeros(mset.shape)
            pset=torch.zeros(mset.shape)
            
            for ii in range(int(nn_samples)):
                
                if setup.mc_dropout:
                    pred = model(data)
                # CMAL has 4 parts: means (m/mu), scales (b), asymmetries (t/) and weights (p/pi):
                    
                if setup.cfg.svitest==1: 
                    import pyro.distributions as dist
                    import pyro
                    import pyro.distributions as dist
                    from pyro.nn import PyroModule
                    ...
                    if setup.cfg.model=="mtslstm":
                        ...
                        if setup.cfg.shared_mtslstm==0:
                            ...
                            for ff in setup.cfg.use_frequencies:
                                ...
                                if setup.cfg.lstmfix==0:
                                    ...
                                    model.lstms[f"{ff}"].bias_hh_l0=sampled_params[f'{ff}.bias_hh_l0']  
                                if  setup.cfg.headfix==0:
                                    model.heads[f"{ff}"].fcs[f'fc1_{ff}'].weight=sampled_params[f"fc1_{ff}.weight"]
                                    model.heads[f"{ff}"].fcs[f'fc1_{ff}'].bias=sampled_params[f"fc1_{ff}.bias"]
                                    model.heads[f"{ff}"].fcs[f'fc2_{ff}'].weight=sampled_params[f"fc2_{ff}.weight"]
                                    model.heads[f"{ff}"].fcs[f'fc2_{ff}'].bias=sampled_params[f"fc2_{ff}.bias"]
                            if  setup.cfg.transferfix==0:
                                model.transfer_fcs["c_1D"].weight=sampled_params['c_1D.weight']
                                model.transfer_fcs["c_1D"].bias=sampled_params['c_1D.bias']
                                model.transfer_fcs["h_1D"].weight=sampled_params['h_1D.weight']
                            ...
                            for ff in setup.cfg.use_frequencies:
                                ...
                                if setup.cfg.lstmfix==0:
                                    ...
                                    model.lstms[f"{ff}"].bias_hh_l0=sampled_params[f'1H.bias_hh_l0']  
                                if  setup.cfg.headfix==0:
                                    model.heads[f"{ff}"].fcs[f'fc1_1D'].weight.data=sampled_params[f"fc1_1D.weight"]
                                    model.heads[f"{ff}"].fcs[f'fc1_1D'].bias.data=sampled_params[f"fc1_1D.bias"]
                                    model.heads[f"{ff}"].fcs[f'fc2_1D'].weight.data=sampled_params[f"fc2_1D.weight"]
                                    model.heads[f"{ff}"].fcs[f'fc2_1D'].bias.data=sampled_params[f"fc2_1D.bias"]
                        ...
                        model.head.fcs['fc1_1D'].weight=sampled_params["fc1_1D.weight"]
                        model.head.fcs['fc1_1D'].bias=sampled_params["fc1_1D.bias"]
                        model.head.fcs['fc2_1D'].weight=sampled_params["fc2_1D.weight"]
                        model.head.fcs['fc2_1D'].bias=sampled_params["fc2_1D.bias"]
                        
                        pred = model(data)    
                        
                    
                mset[:,:,:,ii] = pred[f'mu{freq_suffix}'] # batch, freq,ndis
                sset[:,:,:,ii] = pred[f'sigma{freq_suffix}']
                pset[:,:,:,ii] = pred[f'pi{freq_suffix}']
    
    
     #-----------warm-up-------------------------------------         
            m0=mset[:,:,:,0]              
            m_target0 = _subset_target(m0[:, -frequency_last_n:, :], 0, setup.cfg.n_distributions)
            mask_nan0 = ~torch.isnan(m_target0[:, -1, 0])
            m_sub0 = torch.repeat_interleave(m_target0[mask_nan0, :, :], int(nn_samples), dim=0)
            setlen=m_sub0.shape[0]
            
            m_target_set=torch.zeros((m_target0.shape[0],m_target0.shape[1],m_target0.shape[2],(setup.number_of_targets),nn_samples))
            s_target_set=torch.zeros(m_target_set.shape)
            p_target_set=torch.zeros(m_target_set.shape)
            
            del m0,m_target0
    
            for nth_target in range(setup.number_of_targets):
                
                m_sub=torch.zeros((m_sub0.shape[0]*nn_samples, m_sub0.shape[1],m_sub0.shape[2]))
                s_sub=torch.zeros(m_sub.shape)
                p_sub=torch.zeros(m_sub.shape)           
                ...
                for ii in range(int(nn_samples)):
                    ...
                    m_target_set[:,:,:,nth_target,ii]= m_target
                    s_target_set[:,:,:,nth_target,ii]= s_target 
                    p_target_set[:,:,:,nth_target,ii]= p_target 
                    
                    
                    mask_nan = ~torch.isnan(m_target[:, -1, 0])
                    if any(mask_nan):  # skip if the complete mini-batch is invalid
    
                        m_sub= torch.repeat_interleave(m_target[mask_nan, :, :], nn_samples, dim=0)
                        s_sub = torch.repeat_interleave(s_target[mask_nan, :, :], nn_samples, dim=0)
                        p_sub = torch.repeat_interleave(p_target[mask_nan, :, :], nn_samples, dim=0)
                        ...
                        values = values.view(-1, nn_samples, frequency_last_n).permute(0, 2, 1)
            
                        sample_points[mask_nan, :, nth_target, pp:pp+nn_samples] = values.detach().cpu()
                        pp+=nn_samples                      #16 24 1, 4                         #16,24,2
    
            freq_key = f'y_hat{freq_suffix}'
            samples.update({freq_key: sample_points})
            m_pred.update({f"par{freq_suffix}": m_target_set})
            s_pred.update({f"par{freq_suffix}": s_target_set})
            p_pred.update({f"par{freq_suffix}": p_target_set})
        
        par_pred={"m_pred":m_pred, "s_pred":s_pred, "p_pred":p_pred}
        ...
        for freq_suffix in setup.freq_suffixes:
            ...
            #        256,1,1,100                                256,1,100
            # add sample_points to dictionary of samples:
            freq_key = f'y_hat{freq_suffix}'
            samples.update({freq_key: sample_points})
            m_pred.update({f"par{freq_suffix}": m_target})
            s_pred.update({f"par{freq_suffix}": s_target})
            p_pred.update({f"par{freq_suffix}": p_target})
        
        par_pred={"m_pred":m_pred, "s_pred":s_pred, "p_pred":p_pred}

    return samples, par_pred
#       256,1,1,100,          256,1,3, 
...
def sample_cmal(model: 'BaseModel', data: Dict[str, torch.Tensor], n_samples: int,
    ...
    if (setup.mc_dropout) or (setup.cfg.svitest):
        ...
        if setup.mc_dropout==1:
            model.train()
    
    
        for freq_suffix in setup.freq_suffixes:
            
            pred = model(data) # run pt model to get dim
            m0 = pred[f'mu{freq_suffix}']
            del pred
            ...
            mset=torch.zeros(( m0.shape[0], m0.shape[1],m0.shape[2],nn_samples))
            del m0
            bset=torch.zeros(mset.shape)
            tset=torch.zeros(mset.shape)
            pset=torch.zeros(mset.shape)
            ...
            for ii in range(int(nn_samples)):
                ...
                # CMAL has 4 parts: means (m/mu), scales (b), asymmetries (t/) and weights (p/pi):
                mset[:,:,:,ii] = pred[f'mu{freq_suffix}']
                bset[:,:,:,ii] = pred[f'b{freq_suffix}']
                tset[:,:,:,ii] = pred[f'tau{freq_suffix}']
                pset[:,:,:,ii] = pred[f'pi{freq_suffix}']
     ...
     #-----------warm-up-------------------------------------         
                m0=mset[:,:,:,ii]              
                m_target_0 = _subset_target(m0[:, -frequency_last_n:, :], nth_target, setup.cfg.n_distributions)
                m_target0 = torch.repeat_interleave(m_target_0, int(nn_samples), dim=0)
                
                m_target_set=torch.zeros((m_target_0.shape[0],m_target_0.shape[1],m_target_0.shape[2],nn_samples))
                b_target_set=torch.zeros(m_target_set.shape)
                t_target_set=torch.zeros(m_target_set.shape)
                p_target_set=torch.zeros(m_target_set.shape)
                
                del m0,m_target_0
                
                pp=0
                for ii in range(nn_samples):
                    m=mset[:,:,:,ii]
                    b=bset[:,:,:,ii]
                    t=tset[:,:,:,ii]
                    p=pset[:,:,:,ii]
                    
                    m_target_ = _subset_target(m[:, -frequency_last_n:, :], nth_target, setup.cfg.n_distributions)
                    b_target_ = _subset_target(b[:, -frequency_last_n:, :], nth_target, setup.cfg.n_distributions)
                    t_target_ = _subset_target(t[:, -frequency_last_n:, :], nth_target, setup.cfg.n_distributions)
                    p_target_ = _subset_target(p[:, -frequency_last_n:, :], nth_target, setup.cfg.n_distributions)
    
                    m_target = torch.repeat_interleave(m_target_, nn_samples, dim=0)
                    b_target = torch.repeat_interleave(b_target_, nn_samples, dim=0)
                    t_target = torch.repeat_interleave(t_target_, nn_samples, dim=0)
                    p_target = torch.repeat_interleave(p_target_, nn_samples, dim=0)
    
                    m_target_set[:,:,:,ii]= m_target_ 
                    b_target_set[:,:,:,ii]= b_target_ 
                    t_target_set[:,:,:,ii]= t_target_ 
                    p_target_set[:,:,:,ii]= p_target_ 
                
                
                    # sampling procedure:
                    values = torch.zeros((setup.batch_size_data * nn_samples, frequency_last_n)).to("cpu") #.to(setup.device)
                    values *= torch.tensor(float('nan'))  # set target sample_points to nan
                    for nth_timestep in range(frequency_last_n):
                        
                        mask_nan0 = ~torch.isnan(p_target_[:, nth_timestep, 0])
                    ...
                    values = values.permute(1, 0).reshape(frequency_last_n, -1, nn_samples).permute(1, 0, 2)
                    values = values.detach().cpu()
                    # sample_points.append(values)
                    sample_points[:, :, nth_target, pp:pp+nn_samples]=values
                    pp=pp+nn_samples
                    
                m_target_list.append(m_target_set)
                b_target_list.append(b_target_set)
                t_target_list.append(t_target_set)
                p_target_list.append(p_target_set)
            ...
            freq_key = f'y_hat{freq_suffix}'
            samples.update({freq_key: sample_points})
            m_pred.update({f"par{freq_suffix}":  torch.stack(m_target_list, 2)})
            b_pred.update({f"par{freq_suffix}":  torch.stack(b_target_list, 2)})
            t_pred.update({f"par{freq_suffix}":  torch.stack(t_target_list, 2)})
            p_pred.update({f"par{freq_suffix}":  torch.stack(p_target_list, 2)})
            
        par_pred={"m_pred":m_pred, "b_pred":b_pred, "t_pred":t_pred, "p_pred":p_pred}
        ...
        for freq_suffix in setup.freq_suffixes:
            ...
            for nth_target in range(setup.number_of_targets):
                ...
                m_target = torch.repeat_interleave(m_target_, n_samples, dim=0)
                b_target = torch.repeat_interleave(b_target_, n_samples, dim=0)
                t_target = torch.repeat_interleave(t_target_, n_samples, dim=0)
                p_target = torch.repeat_interleave(p_target_, n_samples, dim=0)
        
                # sampling procedure:
                values = torch.zeros((setup.batch_size_data * n_samples, frequency_last_n)).to("cuda")
                values *= torch.tensor(float('nan'))  # set target sample_points to nan
                for nth_timestep in range(frequency_last_n):
            ...
            freq_key = f'y_hat{freq_suffix}'
            samples.update({freq_key: torch.stack(sample_points, 2)})
            m_pred.update({f"par{freq_suffix}": m_target_})
            b_pred.update({f"par{freq_suffix}": b_target_})
            t_pred.update({f"par{freq_suffix}": t_target_})
            p_pred.update({f"par{freq_suffix}": p_target_})
...
def sample_umal(model: 'BaseModel', data: Dict[str, torch.Tensor], n_samples: int,
    ...
    if (setup.mc_dropout) or (setup.cfg.svitest):
        ...
        if ('y_extended' not in data) & ('y_extended_1H' not in data) & ('y_extended_1D' not in data):
        ...
        for freq_suffix in setup.freq_suffixes:
            ...
            m0=mset[:,:,:,0]
            m_wide0 = torch.cat(m0[:, -frequency_last_n:, :].split(setup.batch_size_data, 0), 2)
            m_wide=torch.zeros((m_wide0.shape[0], m_wide0.shape[1],m_wide0.shape[2],nn_samples)) 
            b_wide=torch.zeros(m_wide.shape)
            t_target=torch.zeros(m_wide.shape)
            ...
            for ii in range(nn_samples):
                ...
                # sampling presets:
                m_wide[:,:,:,ii] = torch.cat(m[:, -frequency_last_n:, :].split(setup.batch_size_data, 0), 2)
                b_wide[:,:,:,ii] = torch.cat(b[:, -frequency_last_n:, :].split(setup.batch_size_data, 0), 2)
    
                t_target[:,:,:,ii] = torch.cat(t[:, -frequency_last_n:, :].split(setup.batch_size_data, 0), 2)
            ...
            m_target = _subset_target(m_wide[:, -frequency_last_n:, :,:], 0, setup.cfg.n_taus)
            m_target_list=torch.zeros((m_target.shape[0], m_target.shape[1],(setup.number_of_targets),m_target.shape[2],nn_samples))
            b_target_list=torch.zeros(m_target_list.shape)
            t_target_list=torch.zeros(m_target_list.shape)
            ...
            for nth_target in range(setup.number_of_targets):
                ...
                m_target_list[:,:,nth_target,:,:]=m_target
                b_target_list[:,:,nth_target,:,:]=b_target
                t_target_list[:,:,nth_target,:,:]=t_target
    
                # sample over n_samples:
                pp=0
                for ii in range(nn_samples):
                    for nth_sample in range(nn_samples):
                        sub_choice = np.random.randint(0, setup.cfg.n_taus)
        
                        mask_nan = ~torch.isnan(m_target[:, 0, 0,0])
                        if any(mask_nan):  # skip computation if entire mini-batch is invalid
                            m_sub = m_target[mask_nan, :, sub_choice,ii]
                            b_sub = b_target[mask_nan, :, sub_choice,ii]
                            t_sub = t_target[mask_nan, :, sub_choice,ii]
                            ...
                            values = values.reshape(-1,frequency_last_n)
                            
                            # add values to sample_points:
                            values = values.detach().cpu()
                            sample_points[mask_nan, -frequency_last_n:, nth_target, pp] = values
                            pp=pp+1
    
            # add sample_points to dictionary of samples:
            freq_key = f'y_hat{freq_suffix}'
            samples.update({freq_key: sample_points})
            m_pred.update({f"par{freq_suffix}": m_target_list})
            b_pred.update({f"par{freq_suffix}": b_target_list})
            t_pred.update({f"par{freq_suffix}": t_target_list})
        
        par_pred={"m_pred":m_pred, "b_pred":b_pred, "t_pred":t_pred}
         ...
         for freq_suffix in setup.freq_suffixes:
             ...
             samples.update({freq_key: sample_points})
             m_pred.update({f"par{freq_suffix}": m_target})
             b_pred.update({f"par{freq_suffix}": b_target})
             t_pred.update({f"par{freq_suffix}": t_target})
