...
class BaseTester(object):
    ...
    def _load_weights(self, epoch: int = None):
        ...
        if self.cfg.bvi==1:
            bb=torch.load(weight_file, map_location=self.device)
            
            if self.cfg.model=="mtslstm":
                self.model.lstms=bb['lstms']
                self.model.heads=bb['heads']
                self.model.transfer_fcs=bb['transfer_fcs']
                
            else:
                self.model.lstm=bb['lstms']
                self.model.head=bb['heads']            
...
#    return preds, obs, dates, mean_losses, all_output, Dist_m,Dist_s,Dist_p
            y_hat, y, dates, all_losses, all_output[basin], all_par = self._evaluate(model, loader, ds.frequencies,
                                                                                save_all_output)
            ...
            for freq in ds.frequencies:
                ...
                # par_vars=self._create_xarray_data_vars(all_m, all_s,all_p)
                par_vars={}
                timedim=dates[lowest_freq][:, -1]
                n_dis=0
                stepdim=np.arange(predict_last_n[freq]) 
                
    #------------------------------------------------------------------------------------------------------------------------ 
                if (self.cfg.mc_dropout==True) or (self.cfg.svitest==1):
                    if self.cfg.head=="gmm":
                    # {"m_pred" "s_pred" "p_pred"}
                        n_tgt=np.shape(all_par[freq]["m_pred"])[2]
                        tgtdim=np.arange((n_tgt))
                        n_dis=np.shape(all_par[freq]["m_pred"])[3]
                        disdim=np.arange((n_dis))
                        n_nns=np.shape(all_par[freq]["m_pred"])[4]
                        nnsdim=np.arange((n_nns))
                        
                        da_m = xarray.DataArray(all_par[freq]["m_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        da_s = xarray.DataArray(all_par[freq]["s_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        da_p = xarray.DataArray(all_par[freq]["p_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        par_vars = {'all_m': da_m, 'all_s': da_s,"all_p": da_p}
    
                    if self.cfg.head=="cmal":
                    # {"m_pred", "b_pred", "t_pred", "p_pred"}
                        n_tgt=np.shape(all_par[freq]["m_pred"])[2]
                        tgtdim=np.arange((n_tgt))
                        n_dis=np.shape(all_par[freq]["m_pred"])[3]
                        disdim=np.arange((n_dis))
                        ...
                        da_m = xarray.DataArray(all_par[freq]["m_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step','n_tgt','n_dis','n_nns'])
                        da_b = xarray.DataArray(all_par[freq]["b_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        da_t = xarray.DataArray(all_par[freq]["t_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        da_p = xarray.DataArray(all_par[freq]["p_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        par_vars = {'all_m': da_m, 'all_b': da_b,"all_t": da_t, "all_p": da_p}
                    
                    if self.cfg.head=="umal":
                    # {"m_pred", "b_pred", "t_pred"}
                        n_tgt=np.shape(all_par[freq]["m_pred"])[2]
                        tgtdim=np.arange((n_tgt))
                        n_dis=np.shape(all_par[freq]["m_pred"])[3]
                        disdim=np.arange((n_dis))
                        ...
                        da_b = xarray.DataArray(all_par[freq]["b_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        da_t = xarray.DataArray(all_par[freq]["t_pred"][:,:,:,:,:], coords=[timedim,stepdim,tgtdim,disdim,nnsdim], dims=['date','step', 'n_tgt','n_dis','n_nns'])
                        par_vars = {'all_m': da_m, 'all_b': da_b,"all_t": da_t}
                    ...
                    if self.cfg.head!="regression":
                        ...
                        pars = xarray.Dataset(data_vars=par_vars, coords=coords)
                        pars = pars.reindex({
                            'date':
                                pd.DatetimeIndex(pd.date_range(pars["date"].values[0], pars["date"].values[-1], freq=lowest_freq),
                                                 name='date')
                        })
                        results[basin][freq]["params"]=pars
            #=================================================================================        
                else:
                    if self.cfg.head=="gmm":
                    # {"m_pred" "s_pred" "p_pred"}
                        n_dis=np.shape(all_par[freq]["m_pred"])[-1]
                        spacedim=np.arange((n_dis))
                        
                        da_m = xarray.DataArray(all_par[freq]["m_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        da_s = xarray.DataArray(all_par[freq]["s_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        da_p = xarray.DataArray(all_par[freq]["p_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        par_vars = {'all_m': da_m, 'all_s': da_s,"all_p": da_p}
                    ...
                    # {"m_pred", "b_pred", "t_pred", "p_pred"}
                        ...
                        da_m = xarray.DataArray(all_par[freq]["m_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        da_b = xarray.DataArray(all_par[freq]["b_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        da_t = xarray.DataArray(all_par[freq]["t_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        da_p = xarray.DataArray(all_par[freq]["p_pred"][:,:,:], coords=[timedim,stepdim, spacedim], dims=['date','step', 'n_distr'])
                        par_vars = {'all_m': da_m, 'all_b': da_b,"all_t": da_t, "all_p": da_p}
    ...
    def _save_results(self, results: Optional[dict], states: Optional[dict] = None, epoch: int = None):
        ...
        if self.cfg.metrics and results is not None:
            ...
            if self.cfg.svitest==0: # original det eva from loaded pt
                metrics_file = parent_directory / f"{self.period}_metrics.csv"
            if self.cfg.svitest==1: # sampling model
                metrics_file = parent_directory / f"{self.period}_metrics_svi.csv"
        ...
        if results is not None:
            ...
            if self.cfg.svitest==0: 
                result_file = parent_directory / f"{self.period}_results.p"
            if self.cfg.svitest==1: 
                result_file = parent_directory / f"{self.period}_results_svi.p"
        ...
        if states is not None:
            ...
            if self.cfg.svitest==1: 
                result_file = parent_directory / f"{self.period}_all_output_svi.p"
    ...
    def _evaluate(self, model: BaseModel, loader: DataLoader, frequencies: List[str], save_all_output: bool = False):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                predictions, loss,par_pre = self._get_predictions_and_loss(model, data)
                losses.append(loss)
                ...
                for freq in frequencies:
                    ...
                    if freq not in preds:
                        ...
                        if self.cfg.head=="gmm":
                        # {"m_pred" "s_pred" "p_pred"}
                            Dist_par["m_pred"] = par_pre["m_pred"][f'par{freq_key}'].cpu()
                            Dist_par["s_pred"] = par_pre["s_pred"][f'par{freq_key}'].cpu()
                            Dist_par["p_pred"] = par_pre["p_pred"][f'par{freq_key}'].cpu()                      
                            Distall_par[freq]=Dist_par
                            
                        if self.cfg.head=="cmal":
                        # {"m_pred", "b_pred", "t_pred", "p_pred"}
                            Dist_par["m_pred"] = par_pre["m_pred"][f'par{freq_key}'].cpu()
                            Dist_par["b_pred"] = par_pre["b_pred"][f'par{freq_key}'].cpu()
                            Dist_par["t_pred"] = par_pre["t_pred"][f'par{freq_key}'].cpu()
                            Dist_par["p_pred"] = par_pre["p_pred"][f'par{freq_key}'].cpu()
                            Distall_par[freq]=Dist_par
                        ...
                        if self.cfg.head=="gmm":
                        # {"m_pred" "s_pred" "p_pred"}
                            Distall_par[freq]["m_pred"] = torch.cat((Distall_par[freq]["m_pred"],par_pre["m_pred"][f'par{freq_key}'].detach().cpu()), 0)
                            Distall_par[freq]["s_pred"] = torch.cat((Distall_par[freq]["s_pred"],par_pre["s_pred"][f'par{freq_key}'].detach().cpu()), 0)
                            Distall_par[freq]["p_pred"] = torch.cat((Distall_par[freq]["p_pred"],par_pre["p_pred"][f'par{freq_key}'].detach().cpu()), 0)
                        if self.cfg.head=="cmal":
                        # {"m_pred", "b_pred", "t_pred", "p_pred"}
                            Distall_par[freq]["m_pred"] = torch.cat((Distall_par[freq]["m_pred"],par_pre["m_pred"][f'par{freq_key}'].detach().cpu()), 0)
                            Distall_par[freq]["b_pred"] = torch.cat((Distall_par[freq]["b_pred"],par_pre["b_pred"][f'par{freq_key}'].detach().cpu()), 0)
                            Distall_par[freq]["t_pred"] = torch.cat((Distall_par[freq]["t_pred"],par_pre["t_pred"][f'par{freq_key}'].detach().cpu()), 0)
                            Distall_par[freq]["p_pred"] = torch.cat((Distall_par[freq]["p_pred"],par_pre["p_pred"][f'par{freq_key}'].detach().cpu()), 0)
                        if self.cfg.head=="umal":
                        # {"m_pred", "b_pred", "t_pred"}
                            Distall_par[freq]["m_pred"] = torch.cat((Distall_par[freq]["m_pred"],par_pre["m_pred"][f'par{freq_key}'].detach().cpu()), 0)
            ...
            for freq in preds.keys():
                ...
                if self.cfg.head=="gmm":
                # {"m_pred" "s_pred" "p_pred"}
                    Distall_par[freq]["m_pred"] = Distall_par[freq]["m_pred"].numpy()
                    Distall_par[freq]["s_pred"] = Distall_par[freq]["s_pred"].numpy()
                    Distall_par[freq]["p_pred"] = Distall_par[freq]["p_pred"].numpy()
                if self.cfg.head=="cmal":
                # {"m_pred", "b_pred", "t_pred", "p_pred"}
                    Distall_par[freq]["m_pred"] = Distall_par[freq]["m_pred"].numpy()
                    Distall_par[freq]["b_pred"] = Distall_par[freq]["b_pred"].numpy()
                    Distall_par[freq]["t_pred"] = Distall_par[freq]["t_pred"].numpy()
                    Distall_par[freq]["p_pred"] = Distall_par[freq]["p_pred"].numpy()
                if self.cfg.head=="umal":
                # {"m_pred", "b_pred", "t_pred"}
                    Distall_par[freq]["m_pred"] = Distall_par[freq]["m_pred"].numpy()
        ...
        return preds, obs, dates, mean_losses, all_output, Distall_par
    ...
    def _get_predictions_and_loss(self, model: BaseModel, data: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, float]:
        ...
        return predictions, {k: v.item() for k, v in all_losses.items()},0
...
class UncertaintyTester(BaseTester):
    ...
    def _get_predictions_and_loss(self, model: BaseModel, data: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, float]:
        ...
        predictions,par_pred = model.sample(data, self.cfg.n_samples)
        model.eval()
        return predictions, {k: v.item() for k, v in all_losses.items()}, par_pred 
