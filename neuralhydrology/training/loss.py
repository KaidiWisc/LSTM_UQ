...
class BaseLoss(torch.nn.Module):
    ...
    def forward(self, prediction: Dict[str, torch.Tensor],
        ...
        loss = torch.nansum(torch.stack(losses))
        total_loss = loss.clone().to("cuda")
        ...
        for reg_module in self._regularization_terms:
            ...
            total_loss += reg_module.weight * reg_out.to("cuda")
...
class MaskedGMMLoss(BaseLoss):
    ...
    def __init__(self, cfg: Config, eps: float = 1e-10):
        ...
        self.eps = eps
        self.obserrormodel_beta=cfg.obserrormodel_beta
        self.obserrormodel_eta=cfg.obserrormodel_eta
        self.obserrormodel_num=cfg.obserrormodel_num
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        mask = ~torch.isnan(ground_truth['y']).cpu() #.any(1).any(1)
        # if torch.nansum(mask)==0: # under this cond. no valus
        #     mask = ~torch.isnan(ground_truth['y']).all(1).all(1)

        maskn=np.repeat(mask, prediction['mu'].shape[2], axis=-1)
        maskn=maskn.to(torch.bool)
        
        # y = ground_truth['y'][mask].cpu()
        y = torch.where(mask, ground_truth['y'].cpu(), torch.tensor(float('nan'))).cpu()
        
        # m = prediction['mu'][maskn].cpu()
        m = torch.where(maskn, prediction['mu'].cpu(), torch.tensor(float('nan'))).cpu()
        
        # s = prediction['sigma'][maskn].cpu()
        s = torch.where(maskn, prediction['sigma'].cpu(), torch.tensor(float('nan'))).cpu()
        
        # p = prediction['pi'][maskn].cpu()
        p = torch.where(maskn, prediction['pi'].cpu(), torch.tensor(float('nan'))).cpu()
        
        result_=torch.zeros((self.obserrormodel_num))
        
        for ii in range(self.obserrormodel_num):
            
            yerr=y*(1+torch.normal(self.obserrormodel_beta, self.obserrormodel_eta,y.shape))
            
            
            result = self._gaussian_distribution(m, s, yerr) * p
            result = torch.sum(result, dim=-1)
            result = -torch.log(result + self.eps)  # epsilon stability
            result_[ii]=torch.nanmean(result)
            
        results=torch.nanmean(result_)
...
class MaskedCMALLoss(BaseLoss):
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        ...
        # y = ground_truth['y'][mask].cpu()
        y = torch.where(mask, ground_truth['y'].cpu(), torch.tensor(float('nan'))).cpu()
        # m = prediction['mu'][mask].cpu()
        m = torch.where(maskn, prediction['mu'].cpu(), torch.tensor(float('nan'))).cpu()
        # b = prediction['b'][mask].cpu()
        b = torch.where(maskn, prediction['b'].cpu(), torch.tensor(float('nan'))).cpu()
        # t = prediction['tau'][mask].cpu()
        t = torch.where(maskn, prediction['tau'].cpu(), torch.tensor(float('nan'))).cpu()
        # p = prediction['pi'][mask].cpu()
        p = torch.where(maskn, prediction['pi'].cpu(), torch.tensor(float('nan'))).cpu()
        
        result=torch.zeros((self.obserrormodel_num))
        ...
        for ii in range(self.obserrormodel_num):
            ...
            result_ = torch.logsumexp(log_weights + log_like, dim=2)  # non if 
            result[ii] = -torch.nanmean(torch.nansum(result_, dim=1))
        
        results=torch.nanmean(result)
...
class MaskedUMALLoss(BaseLoss):
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        mask = ~torch.isnan(ground_truth['y_extended']).cpu() #.any(1).any(1)
        # if torch.nansum(mask)==0: # under this cond. no valus
        #     mask = ~torch.isnan(ground_truth['y_extended']).all(1).all(1)
        
        maskn=np.repeat(mask, prediction['mu'].shape[2], axis=-1)
        maskn=maskn.to(torch.bool)
        
        # y = ground_truth['y_extended'][mask].cpu()
        y = torch.where(mask, ground_truth['y_extended'].cpu(), torch.tensor(float('nan'))).cpu()
        # t = ground_truth['tau'][mask].cpu()
        t = torch.where(maskn, ground_truth['tau'].cpu(), torch.tensor(float('nan'))).cpu()
        # m = prediction['mu'][mask].cpu()
        m = torch.where(maskn, prediction['mu'].cpu(), torch.tensor(float('nan'))).cpu()
        # b = prediction['b'][mask].cpu()
        b = torch.where(maskn, prediction['b'].cpu(), torch.tensor(float('nan'))).cpu()
        ...
        for ii in range(self.obserrormodel_num):
            ...
            result_[ii]= -torch.nanmean(torch.nansum(result, dim=1))
